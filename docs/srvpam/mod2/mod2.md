...
2. An account Standard User (**user**), that we have been using throughout the lab, that is also a normal user, but due to her role in the organisation can update some packages (wget and zip) and control the Mail server (postfix based) that is running on this server. For the rest this user has no extra privileges.
...
![JIT Commands](../../images/lab0003.png)
...
Try to do the same on the DC1 and see that you can login using the same credentials. Hence the account is capable to login to the DC and should, by default, be able to log in to the RDS01, but a configuration is blocking the access.

![JIT Commands](images/lab009.png)

Sign out of the DC1

Back at the Client, try using the Remote Desktop Connection (**dc1**) and notice that the same message appears as on the console **The account is not authorized to log in from this station**. Close the connection, or wait a few more seconds and it will close by itself.
...
The user is getting the first MFA challenge, retype the **Password**. The next MFA challenge is a choice to answer **Security Questions** or use **Email**. This time selct the Email option and click **Next**. 

![](../../images/lab0005.png)

Open MS Mail on the Client VM and click the link **Continue with Authentication**. This will open a Chrome session with the message that the Authentication was successful and return to the RDS01 server. 

The Eventviewer will have opened and you can open the security information. Return to the other Eventviewer and refresh the page on the security information. This is still blocked as having no access. So only the second Eventviewer is running with elevated privileges. 
